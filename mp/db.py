...
from simpleBDB import AbortTXNException, retry, txnAbortOnError
...
def openEnv(recover=False):
    db.open_env()
    db.createEnvWithDir(dbPath, recover=recover)
    db.setLockDetect()
...
def cleanLogs():
    ...
    logBackupDir = os.path.join('data', 'db_log_backup')
...
@retry
@txnAbortOnError
def updateFiles(data, txn=None):
    modelSummaryKeys = ModelSummaries.db_key_tuples()

    for key, hub in HubInfo.all_dict(txn=txn).items():
        user, hubName = key
        genome = hub['genome']

        chromSizePath = checkChromSize(genome)

        if chromSizePath is None: continue
...
def checkChromSize(genome):
    genomesPath = os.path.join(cfg.jbrowsePath, cfg.dataPath, 'genomes', genome)

    genomeFile = os.path.join(genomesPath, genome + '.chrom.sizes')
    url = 'http://hgdownload.soe.ucsc.edu/goldenPath/%s/bigZips/%s.chrom.sizes' % (genome, genome)
